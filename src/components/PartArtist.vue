<template>
  <div class="media">
    <div class="media-content fd-has-action is-clipped" v-on:click="open_artist_albums">
      ...
      <h2 class="subtitle is-7">{{ artist.album_count }} albums | {{ artist.song_count }} songs</h2>
    </div>
  ...
  methods: {
    ...
    queue_add: function () {
      webapi.queue_add(this.artist.uri).then(
        ...
        this.$store.dispatch('add_notification', { text: 'Artist tracks appended to queue', timeout: 3000 })
      )
    }
  }
}
