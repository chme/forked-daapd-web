...
export default {
  ...
  methods: {
    open_album_tracks: function () {
      this.$router.push({ path: '/albums/' + this.album.persistent_id + '/tracks', query: { context_uri: this.album.uri } })
    },
    ...
    queue_add: function () {
      webapi.queue_add(this.album.uri).then(
        ...
        this.$store.dispatch('add_notification', { text: 'Album tracks appended to queue', timeout: 3000 })
      )
    }
  }
}
